...
## 📋 Документация по трансформациям JSON

**[📖 JSON Payload Трансформации: OpenAI ↔ GigaChat](docs/GIGACHAT_COMPATIBILITY.md)**

Подробная документация, объясняющая **почему GigaChat несовместим с OpenAI API** и как работает двунаправленный трансформер:

- **Проблема совместимости**: GigaChat API использует `functions` вместо `tools`, `function_call` вместо `tool_calls`, и не поддерживает массивы в `content`
- **Решение**: Автоматическая трансформация запросов и ответов для полной совместимости
- **Максимально полные примеры**: Реальные JSON payload с пошаговыми объяснениями трансформаций
- **Специальные случаи**: Обработка streaming, ошибок, сложных структур данных

Этот трансформер позволяет использовать GigaChat через стандартный OpenAI интерфейс без изменения клиентского кода.
...
1. **Откройте настройки Cline**:
   - Нажмите на иконку ⚙️ (шестеренка) в панели Cline
   - Или используйте команду `Cmd+Shift+P` → "Cline: Open Settings"

2. **Выберите API Provider**:
   - В поле "API Provider" выберите `OpenAI Compatible` (либо `LiteLLM`)
   - Это позволит использовать наш прокси-сервер

3. **Настройте Base URL**:
   - В поле "Base URL" введите: `http://localhost:4000`
   - ⚠️ **Важно**: Не добавляйте слэш в конце URL

4. **Установите API Key**:
   - В поле "API Key" введите любое значение, например: `gigachat-key`
   - Реальный токен управляется автоматически нашим прокси

5. **Выберите модель**:
   - В поле "Model" введите: `gigachat`
   - Или выберите другую модель из списка выше
